...
//
// appsv1Guestbook tests
//
func appsv1GuestbookChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(guestbook.CreateFuncs) == 0 {
		return nil
	}

	workload, err := guestbook.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := guestbook.Generate(*workload)
	if err != nil {
...
func appsv1GuestbookNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &appsv1.Guestbook{},
		sampleManifestFile: "../../config/samples/apps_v1_guestbook.yaml",
		getChildrenFunc:    appsv1GuestbookChildrenFuncs,
		logSyntax:          "controllers.apps.Guestbook",
	}
}

func (tester *E2ETest) appsv1GuestbookTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_appsv1Guestbook() {
	tester := appsv1GuestbookNewHarness("")
	tester.appsv1GuestbookTest(testSuite)
}
