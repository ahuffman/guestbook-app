...
func NewGuestbookSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc: GenerateGuestbook, UseWorkloadManifest: true,
		WorkloadKind: "Guestbook",
	}

	generateCmd.Setup()
}

// GenerateGuestbook runs the logic to generate child resources for a
// Guestbook workload.
func GenerateGuestbook(g *cmdgenerate.GenerateSubCommand) error {
